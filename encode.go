...
func (bsm ByteSliceMode) encodingTag() (uint64, error) {
	...
	return 0, errors.New("cbor: invalid ByteSliceMode " + strconv.Itoa(int(bsm)))
}
...
const (
	// ByteArrayAsByteSlice encodes byte arrays the same way that a byte slice with identical
	// length and contents is encoded.
	ByteArrayAsByteSlice = iota
...
func (ae arrayEncodeFunc) encode(e *encoderBuffer, em *encMode, v reflect.Value) error {
	if em.byteArray == ByteArrayAsByteSlice && v.Type().Elem().Kind() == reflect.Uint8 {
