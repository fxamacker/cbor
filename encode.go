...
func (tme textMarshalerEncoder) encode(e *bytes.Buffer, em *encMode, v reflect.Value) error {
	...
	if err != nil {
		return fmt.Errorf("cbor: non-nil error returned from MarshalText of %s: %w", vt, err)
	}
	if b := em.encTagBytes(vt); b != nil {
		e.Write(b)
	}
...
func (tme textMarshalerEncoder) isEmpty(em *encMode, v reflect.Value) (bool, error) {
	...
	data, err := m.MarshalText()
	if err != nil {
		return false, fmt.Errorf("cbor: non-nil error returned from MarshalText of %s: %w", v.Type(), err)
	}
	return len(data) == 0, nil
}
