...
func FuzzStream(f *testing.F) {
	...
	f.Add([]byte{0x01, 0x3e})
	f.Add(hexDecode("83010203"))
	f.Add([]byte{0xc2, 0x49, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
	f.Add([]byte{0xc2, 0x49, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc3, 0x49, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
	f.Add([]byte{0xc2, 0x49, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x61, 0x62, 0x63})

	f.Fuzz(func(t *testing.T, input_data []byte) {
		decoder := NewDecoder(bytes.NewReader(input_data))
		var v1 interface{}
		_ = decoder.Decode(&v1)
	})
}
